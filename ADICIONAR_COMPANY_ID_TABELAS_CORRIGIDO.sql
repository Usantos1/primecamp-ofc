-- =====================================================
-- ADICIONAR company_id NAS TABELAS EXISTENTES (VERSÃO CORRIGIDA)
-- Migração para multi-tenancy
-- =====================================================

-- Verificar se tabela companies existe ANTES de executar
DO $$ 
DECLARE
...
BEGIN
    ...
    IF NOT companies_exists THEN
        RAISE EXCEPTION 'Tabela companies não existe! Execute primeiro CRIAR_SISTEMA_REVENDA_CORRIGIDO.sql';
    END IF;
    
    -- Verificar se empresa admin existe
    IF NOT EXISTS (SELECT 1 FROM public.companies WHERE id = admin_company_id) THEN
        RAISE EXCEPTION 'Empresa admin não encontrada! Execute primeiro CRIAR_SISTEMA_REVENDA_CORRIGIDO.sql';
    END IF;
    
    RAISE NOTICE 'Verificações passadas. Iniciando migração...';
END $$;
