...
interface DiscQuestionsDetailProps {
  responses: DiscResponse[];
  dominantProfile: string;
}

// Same questions structure as useCandidateDiscTest
...
export const DiscQuestionsDetail = ({ responses, dominantProfile }: DiscQuestionsDetailProps) => {
  const getProfileColor = (type: 'D' | 'I' | 'S' | 'C') => {
    const colors = {
      'D': 'bg-red-500',
      'I': 'bg-yellow-500',
      'S': 'bg-green-500',
  ...
  const getTypeLabel = (type: 'D' | 'I' | 'S' | 'C') => {
    const labels = {
      'D': 'Dominante',
      'I': 'Influente',
      'S': 'Estável', 
  ...
  const responseMap = responses.reduce((acc, response) => {
    acc[response.questionId] = response.selectedType;
    return acc;
  }, {} as Record<number, 'D' | 'I' | 'S' | 'C'>);
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {['D', 'I', 'S', 'C'].map((type) => {
              const count = responses.filter(r => r.selectedType === type).length;
              return (
                <div key={type} className="text-center">
                  <div className={`w-12 h-12 mx-auto mb-2 rounded-full flex items-center justify-center text-white font-bold ${getProfileColor(type as 'D' | 'I' | 'S' | 'C')}`}>
                    {count}
                  </div>
                  <div className="text-sm font-medium">{getTypeLabel(type as 'D' | 'I' | 'S' | 'C')}</div>
                </div>
              );
            })}
          </div>
          <div className="text-center">
            <Badge variant="outline" className="text-sm">
              Perfil Dominante: <strong>{getTypeLabel(dominantProfile as 'D' | 'I' | 'S' | 'C')}</strong>
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-4">
        {DISC_QUESTIONS.map((question) => {
          const selectedType = responseMap[question.id];
            ...
            <Card key={question.id}>
              ...
              <CardContent>
                <div className="grid gap-3">
                  {Object.entries(question.options).map(([type, option]) => {
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3 rounded-lg border-2 transition-colors ${
                          isSelected 
                            ? `border-current ${getProfileColor(type as 'D' | 'I' | 'S' | 'C')} text-white` 
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                            isSelected 
                              ? 'bg-white text-black' 
                              : `${getProfileColor(type as 'D' | 'I' | 'S' | 'C')} text-white`
                          }`}>
                            {type}
                          </div>
                        ...
                        {isSelected && (
                          <div className="mt-2 ml-11">
                            <Badge variant="secondary" className="text-xs">
                              Resposta Selecionada - {getTypeLabel(type as 'D' | 'I' | 'S' | 'C')}
                            </Badge>
                          </div>
                        )}
                      </div>
