...
interface EstoqueMovimentacao {
  id: string;
  data: string;
  ref_tipo: 'OS' | 'Venda' | 'Pedido' | 'Cancelamento' | 'Devolução' | 'Ajuste' | 'Troca' | 'Perda' | 'Devolução OS';
  ref_id?: string;
  ref_numero: number;
    ...
    if (internalMovs && internalMovs.length > 0) {
      (internalMovs as any[]).forEach((m) => {
        ...
        if (isDevolucaoOS) {
          ...
          refTipo = 'Venda';
        } else if (tipoMov.includes('pedido') || String(m.motivo || '').toLowerCase().includes('pedido')) {
          refTipo = 'Pedido';
...
export function ProductFormOptimized({
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Não foi possível salvar o produto. Confira os dados e tente novamente.';
      toast({
                      ...
                      <TableBody>
                        {movimentacoes.map((mov) => {
                          ...
                          const tipoBadgeColors: Record<string, string> = {
                            ...
                            'Pedido': 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300',
